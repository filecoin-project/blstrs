...
#[bench]
fn bench_fp12_compress2(b: &mut ::test::Bencher) {
    const SAMPLES: usize = 1000;
    ...
    let mut count = 0;
    b.iter(|| {
        let tmp = v[count].compress2().unwrap();
        count = (count + 1) % SAMPLES;
        tmp
    });
}
...
#[bench]
fn bench_fp12_uncompress2(b: &mut ::test::Bencher) {
    const SAMPLES: usize = 1000;
    ...
    let v: Vec<_> = (0..SAMPLES)
        .map(|_| {
            let p = G1Projective::random(&mut rng).into_affine();
            let q = G2Projective::random(&mut rng).into_affine();
            blstrs::pairing(p, q).compress2().unwrap()
        })
        .collect();
