...
#[bench]
fn bench_scalar_repr_add_nocarry(b: &mut ::test::Bencher) {
    ...
    let v: Vec<(ScalarRepr, ScalarRepr)> = (0..SAMPLES)
        .map(|_| {
            let mut tmp1 = Scalar::random(&mut rng).into_repr();
            let mut tmp2 = Scalar::random(&mut rng).into_repr();
...
#[bench]
fn bench_scalar_repr_sub_noborrow(b: &mut ::test::Bencher) {
    const SAMPLES: usize = 1000;
    ...
    let v: Vec<(ScalarRepr, ScalarRepr)> = (0..SAMPLES)
        .map(|_| {
            let tmp1 = Scalar::random(&mut rng).into_repr();
            let mut tmp2 = tmp1;
...
#[bench]
fn bench_scalar_repr_num_bits(b: &mut ::test::Bencher) {
    const SAMPLES: usize = 1000;
    ...
    let v: Vec<ScalarRepr> = (0..SAMPLES)
        .map(|_| Scalar::random(&mut rng).into_repr())
        .collect();
...
#[bench]
fn bench_scalar_repr_mul2(b: &mut ::test::Bencher) {
    const SAMPLES: usize = 1000;
...
#[bench]
fn bench_scalar_repr_div2(b: &mut ::test::Bencher) {
    const SAMPLES: usize = 1000;
...
#[bench]
fn bench_scalar_into_repr(b: &mut ::test::Bencher) {
    const SAMPLES: usize = 1000;
